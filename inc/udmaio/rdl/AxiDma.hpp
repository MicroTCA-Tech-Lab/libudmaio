...
struct mm2s_curdesc_t {
    ...
    uint32_t current_descriptor_pointer : 26;
} _HECTARE_PACKED;
...
struct mm2s_taildesc_t {
    ...
    uint32_t tail_descriptor_pointer : 26;
} _HECTARE_PACKED;
 ...
 *
 * @tparam R_acc Register accessor template (provided by user), receiving the register type as parameter
 * @tparam R_acc_arg Constructor argument type for the register accessor (e.g. hardware-dependent information)
 *
 */
template <template <typename, uint32_t> typename R_acc, typename R_acc_arg>
 ...
 */
template <template <typename, uint32_t> typename R_acc>
using block = block_template<R_acc, typename R_acc<mm2s_dmacr_t, 0>::constructor_arg_type>;
